...
import com.github.mrmks.mc.status.adapt.*;
import com.github.mrmks.mc.status.api.*;
import com.github.mrmks.mc.status.utils.*;
...
public class Registry {
    ...
    static <T> BakeResult<T> bakeAll(Collection<IExtension<T>> extensions, IDataAccessor ida, IEntityConvert<T> iec, ILogger logger, IGuiCallback guiCallback, ConfigMap configMap) {
        ...
        int[] intervals = new int[16];
        {
            ...
            for (IndexedAttribute<T> ia : attributeMap.values()) {
                if (ia.flag == 0) {
                    ...
                    attributeAry[resourceSize + i] = new IndexedAttribute<>(ia.attribute, ia.name.concat(SUFFIX_BOUND), (byte) (FLAG_RESOURCE | FLAG_SPECIAL));
                    attributeAry[resourceSize + i].index = (short) (resourceSize + i);
                    attributeAry[rs2 + i] = new IndexedAttribute<>(ia.attribute, ia.name.concat(SUFFIX_STEP), (byte) (FLAG_RESOURCE | FLAG_SPECIAL | FLAG_STEP));
        ...
        EntityManager<T> em = new EntityManager<>(tm, iec, ida, stateControl, attributeAry, resourceSize, intervals, intervalIds, attributeProviderAry);
        HandlerList<T> hl = new HandlerList<>(stateControl, em, bm, handlerAry, modifierAry, wrappedHandlerMap, paramMap, resourceSize);
        bm.setEntityManager(em);
        bm.setHandlerList(hl);
    ...
    private static class IndexedAttribute<T> extends WrappedAttribute<T> implements IndexedEntry {
        ...
        @Override
        public WrappedAttribute<T> copy() {
            IndexedAttribute<T> copied = new IndexedAttribute<>(attribute, name, flag);
            copied.index = index;
            copied.dataIndex = dataIndex;
            return copied;
        }
