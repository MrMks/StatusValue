...
class EntityManager<T> {
    ...
    private void removeEntity0(StatusEntity se) {
        ...
        for (int i = 0; i < resourceSize; i ++) val[i] = table.get(i) - baseValues[i];
        IEntityDataAccessor da = dataAccessor.withEntity(se.storeKey);
        da.writeValue(val);
    ...
    private void applyResource0(StatusEntity se, T entity, int[] id, int[] val, boolean step) {
        ...
        for (int i = 0; i < id.length; ++i) {
            ...
            if (_mod > 0) {
                _mod = Math.min(table.get(_id + resourceSize), _mod);
            } else {
    ...
    void tick() {
        while (!removingQueue.isEmpty()) removeEntity(removingQueue.remove());
    }
